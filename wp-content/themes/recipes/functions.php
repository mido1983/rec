<?php
/**
 * recipes functions and definitions
 *
...
if ( ! function_exists( 'recipes_setup' ) ) :
	...
	function recipes_setup() {
		 ...
		 * If you're building a theme based on recipes, use a find and replace
		 * to change 'recipes' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'recipes' ),
            'menu-5' => esc_html__( 'Primary Content', 'recipes' ),
            'menu-2' => esc_html__( 'Footer', 'recipes' ),
            'menu-3' => esc_html__( 'Left Sidebar', 'recipes' ),
            'menu-4' => esc_html__( 'Right Sidebar', 'recipes' ),
		...
		add_theme_support( 'custom-background', apply_filters( 'recipes_custom_background_args', array(
 ...
 */
function recipes_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'recipes_content_width', 640 );
}
add_action( 'after_setup_theme', 'recipes_content_width', 0 );
...
wp_enqueue_style( 'responsive', get_template_directory_uri().'/css/responsive/responsive.css',array('recipes-style')  );
	...
	wp_enqueue_script( 'recipes-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );
	wp_enqueue_script( 'recipes-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}

	//wp_enqueue_script( 'jquery', get_template_directory_uri() . '/js/jquery/jquery-2.2.4.min.js', array('jquery') );
	...
	wp_enqueue_script( 'plugins', get_template_directory_uri() . '/js/others/plugins.js', array('jquery'),'',TRUE );
	wp_enqueue_script( 'active', get_template_directory_uri() . '/js/active.js', array('jquery'),'',TRUE );
	wp_enqueue_script( 'recipes-custom', get_template_directory_uri() . '/js/custom.js', array('jquery'),'',TRUE );
